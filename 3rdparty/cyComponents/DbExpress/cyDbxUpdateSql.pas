  ...
  try
    ...
    begin
      aQuery.SQL.Text := 'SELECT DISTINCT TABLE_NAME FROM information_schema.columns WHERE TABLE_SCHEMA = '
                          + SQLGetStringExpr(cyDbxUpdateSql.FSQLConnection.Params.Values['Database'], GetDbxConnectionDriver(cyDbxUpdateSql.FSQLConnection))
                          + ' AND TABLE_NAME = ' + SQLGetStringExpr(FTableName, GetDbxConnectionDriver(cyDbxUpdateSql.FSQLConnection));
    end
    else begin
      // Not working on MySQL prior 5.x and on Delphi 7:
      aQuery.SQL.Text := 'SHOW TABLES LIKE ' + SQLGetStringExpr(FTableName, GetDbxConnectionDriver(cyDbxUpdateSql.FSQLConnection));
    end;
  ...
  try
    ...
    begin
      aQuery.SQL.Text := 'SELECT COLUMN_NAME FROM information_schema.columns WHERE TABLE_SCHEMA = '
                           + SQLGetStringExpr(cyDbxUpdateSql.FSQLConnection.Params.Values['Database'], GetDbxConnectionDriver(cyDbxUpdateSql.FSQLConnection))
                           + ' AND TABLE_NAME = ' + SQLGetStringExpr(FTableName, GetDbxConnectionDriver(cyDbxUpdateSql.FSQLConnection))
                           + ' AND COLUMN_NAME = ' + SQLGetStringExpr(FColumnName, GetDbxConnectionDriver(cyDbxUpdateSql.FSQLConnection));
    end
    else begin
      // Not working on MySQL prior 5.x :
      aQuery.SQL.Text := 'SHOW COLUMNS FROM ' + FTableName + ' LIKE ' + SQLGetStringExpr(FColumnName, GetDbxConnectionDriver(cyDbxUpdateSql.FSQLConnection));
    end;
